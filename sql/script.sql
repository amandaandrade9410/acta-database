...
CREATE TABLE IF NOT EXISTS empresa (
    ...
    nome VARCHAR(160) NOT NULL CHECK (nome IS NULL OR LENGTH(TRIM(nome)) >= 2),
...
CREATE TABLE IF NOT EXISTS pdca.ciclo (
    id BIGSERIAL PRIMARY KEY,
    id_empresa BIGINT NOT NULL REFERENCES empresa(id) ON DELETE CASCADE,
    id_responsavel BIGINT NOT NULL REFERENCES colaborador(id),
...
CREATE TABLE IF NOT EXISTS pdca.meta (
    ...
    valor_base NUMERIC(15,2) CHECK ((valor_base IS NULL OR valor_base >= 0)),
    valor_alvo NUMERIC(15,2) CHECK ((valor_alvo IS NULL OR valor_alvo >= 0)),
    unidade VARCHAR(30),
    prazo DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS pdca.plano_acao (
    ...
    criado_por BIGINT REFERENCES usuario_sistema(id),
    criado_em TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    atualizado_em TIMESTAMPTZ
);
...
CREATE TABLE IF NOT EXISTS pdca.treinamento (
    ...
    id_responsavel BIGINT REFERENCES colaborador(id),
    titulo VARCHAR(160) NOT NULL,
    descricao TEXT,
...
CREATE TABLE IF NOT EXISTS pdca.plano_5w2h (
    ...
    id_who_responsavel BIGINT NOT NULL REFERENCES colaborador(id),
...
CREATE TABLE IF NOT EXISTS pdca.problema (
    ...
    descricao TEXT NOT NULL,
    peso NUMERIC(2,2) NOT NULL CHECK (peso >= 0 AND peso <= 10),
...
CREATE TABLE IF NOT EXISTS pdca.colaborador_ciclo (
    id BIGSERIAL PRIMARY KEY,
    id_colaborador BIGINT NOT NULL REFERENCES colaborador(id) ON DELETE CASCADE,
    id_ciclo BIGINT NOT NULL REFERENCES pdca.ciclo(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS pdca.colaborador_treinamento (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS pdca.priorizacao_problema_colaborador (
    id BIGSERIAL PRIMARY KEY,
    id_problema BIGINT NOT NULL REFERENCES pdca.problema(id) ON DELETE CASCADE,
    id_colaborador BIGINT NOT NULL REFERENCES colaborador(id) ON DELETE CASCADE,
    posicao INTEGER NOT NULL CHECK (posicao > 0),
    peso_calculado NUMERIC(5,2) NOT NULL CHECK (peso_calculado >= 0),
    criado_em TIMESTAMPTZ NOT NULL DEFAULT NOW(),
